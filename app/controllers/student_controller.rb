class StudentController < ApplicationController
    ...
    if request.post?
      ...
      if @student.save
        ...
        if sms_setting.application_sms_active and @student.is_sms_enabled
          recipients = []
          message = t('student.admissionDone')+" #{@student.admission_no} "+t('student.passwordIS')+" #{@student.admission_no}123"
        ...
        flash[:notice] = t('student.recordSavedFillParent')
  ...
  def search_ajax
    if params[:option] == "active"
      @students = Student.first_name_or_middle_name_or_last_name_or_admission_no_like_any params[:query].split unless params[:query].empty?
      render :layout => false
    else
      @archived_students = ArchivedStudent.first_name_or_middle_name_or_last_name_or_admission_no_like_any params[:query].split unless params[:query].empty?
  ...
  def category_delete
    @student_category = StudentCategory.destroy(params[:id])
    @student_categories = StudentCategory.find :all
  end
