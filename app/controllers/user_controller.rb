    ...
    if request.post?
      ...
      if User.authenticate?(@user.username, params[:user][:old_password])
        if params[:user][:new_password] == params[:user][:confirm_password]
          ...
          flash[:warn_notice] = t('passwordCombinationNOK')
        end
      else
        flash[:warn_notice] = t('oldPasswordNOK')
      end
    end
  end
    ...
    if request.post?
      if params[:user][:new_password]=='' and params[:user][:confirm_password]==''
        ...
        if params[:user][:new_password] == params[:user][:confirm_password]
          ...
          flash[:warn_notice] = t('passwordConfirmationNOK')
        end
      end
...
#    redirect_to :action=>"login"
    ...
    if request.post? and params[:reset_password]
      if user = User.find_by_email(params[:reset_password][:email])
        ...
        flash[:notice] = t('passwordLinkEmailed')+" #{user.email}"
        redirect_to :action => "index"
      else
        flash[:notice] = t('noUserEmail')+" #{params[:reset_password][:email]}"
      end
    end
  end
  ...
  def login
    ...
    if request.post? and params[:user]
      ...
      if user and User.authenticate?(@user.username, @user.password)
        ...
        flash[:notice] = t("welcome.title")+", #{user.first_name} #{user.last_name}!"
        redirect_to :controller => 'user', :action => 'dashboard'
      else
        flash[:notice] = t('invalidUserPass')
      end
    end
  end
  ...
  def profile
    ...
    @student = Student.find_by_admission_no(@user.username)
    if @user.nil?
      flash[:notice] = t("flash.UserProfileNotFound")
      redirect_to :action => 'dashboard'
    end
  end
  ...
  def reset_password
    ...
    if user
      if user.reset_password_code_until > Time.now
        ...
        flash[:notice] = t('resetTimeExpired')
        redirect_to :action => 'index'
      end
    else
      flash[:notice]= t('invalidResetLink')
      redirect_to :action => 'index'
    end
  end
    ...
    if request.post?
      ...
      if user
        if params[:set_new_password][:new_password] === params[:set_new_password][:confirm_password]
          ...
          flash[:notice] = t('passwordNOK')
          redirect_to :action => 'set_new_password', :id => user.reset_password_code
        end
      else
        flash[:notice] = t('invalidLink')
        redirect_to :action => 'index'
      end
    end
  end
