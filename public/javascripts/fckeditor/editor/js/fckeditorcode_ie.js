...
FCKIECleanup.prototype.AddItem = function(A, B) {this.Items.push([A,B]);};
function FCKIECleanup_Cleanup() {
    if (!this._FCKCleanupObj || !window.FCKUnloadFlag) return;
...
;
function Doc_OnSelectionChange() {if (!FCK.IsSelectionChangeLocked && FCK.EditorDocument) FCK.Events.FireEvent("OnSelectionChange");}
;
function Doc_OnDrop() {
...
;
function Document_OnContextMenu() {return (event.srcElement._FCKShowContextMenu == true);}
;
...
FCK.PasteAsPlainText = function(A) {
    ...
    ;
    var B = null;
    if (!A) B = clipboardData.getData("Text"); else B = A;
    if (B && B.length > 0) {
...
FCK._CheckIsPastingEnabled = function(A) {
    ...
    if (FCK._PasteIsEnabled) {if (!A) B = true;} else B = false;
    delete FCK._PasteIsEnabled;
    return B;
};
function FCK_CheckPasting_Listener() {FCK._PasteIsEnabled = true;}
;
    ...
    if (A.length > 0) {
        ...
        for (i = 0; i < G.length; i++) {
            ...
            if (D.getAttribute('href', 2) == F) {
                ...
                D.innerHTML = I;
                var J = D.lastChild;
                if (J && J.nodeName == 'BR') {FCKDomTools.InsertAfterNode(D, D.removeChild(J));}
                ;
                C.push(D);
            }
        }
...
function _FCK_RemoveDisabledAtt() {this.removeAttribute('disabled');}
;
...
FCKTools.CancelEvent = function(e) {return false;};
FCKTools._AppendStyleSheet = function(A, B) {return A.createStyleSheet(B).owningElement;};
...
FCKTools.ClearElementAttributes = function(A) {A.clearAttributes();};
...
FCKTools.CreateXmlObject = function(A) {
    var B;
    switch (A) {case 'XmlHttp':try { return new XMLHttpRequest();} catch (e) {};B = ['MSXML2.XmlHttp','Microsoft.XmlHttp'];break;case 'DOMDocument':B = ['MSXML2.DOMDocument','Microsoft.XmlDom'];break;
    }
    ;
    for (var i = 0; i < 2; i++) {try { return new ActiveXObject(B[i]);} catch (e) {}}
    ;
...
FCKTools.DisableSelection = function(A) {
    ...
    var e,i = 0;
    while ((e = A.all[i++])) {
        switch (e.tagName) {case 'IFRAME':case 'TEXTAREA':case 'INPUT':case 'SELECT':break;default:e.unselectable = 'on';
        }
    }
};
...
FCKTools.AddEventListener = function(A, B, C) {A.attachEvent('on' + B, C);};
FCKTools.RemoveEventListener = function(A, B, C) {A.detachEvent('on' + B, C);};
FCKTools.AddEventListenerEx = function(A, B, C, D) {
    var o = {};
    o.Source = A;
    o.Params = D || [];
    o.Listener = function(ev) {return C.apply(o.Source, [ev].concat(o.Params));};
...
FCKTools.GetViewPaneSize = function(A) {
    ...
    if (C && C.clientWidth) B = C; else B = A.document.body;
...
FCKTools.AppendElement = function(A, B) {return A.appendChild(this.GetElementDocument(A).createElement(B));};
FCKTools.ToLowerCase = function(A) {return A.toLowerCase();};
var FCKeditorAPI;
...
FCKXHtml._GetMainXmlString = function() {return this.MainNode.xml;};
FCKXHtml._AppendAttributes = function(A, B, C, D) {
    ...
    for (var n = 0; n < E.length; n++) {
        ...
        if (F.specified) {
            ...
            var H;
            if (G.StartsWith('_fck')) continue; else if (G == 'style') {
                var I = FCKTools.ProtectFormStyles(B);
                H = B.style.cssText.replace(FCKRegexLib.StyleProperties, FCKTools.ToLowerCase);
                FCKTools.RestoreFormStyles(B, I);
            } else if (G == 'class') {
                H = F.nodeValue.replace(FCKRegexLib.FCK_Class, '');
                if (H.length == 0) continue;
            } else if (G.indexOf('on') == 0) H = F.nodeValue; else if (D == 'body' && G == 'contenteditable') continue; else if (F.nodeValue === true) H = G; else {try {H = B.getAttribute(G, 2);} catch (e) {}}
            ;
...
FCKDomRange.prototype.Select = function(A) {if (this._Range) this.SelectBookmark(this.CreateBookmark(true), A);};
FCKDomRange.prototype.SelectBookmark = function(A, B) {
    ...
    if (G) {
        ...
        D = (B || !F.previousSibling || F.previousSibling.nodeName.toLowerCase() == 'br') && !F.nextSibing;
        E = this.Window.document.createElement('span');
        E.innerHTML = '&#65279;';
        F.parentNode.insertBefore(E, F);
        if (D) {F.parentNode.insertBefore(this.Window.document.createTextNode('\ufeff'), F);}
    }
    ;
    if (!this._Range) this._Range = this.CreateRange();
...
FCKDomRange.prototype._GetSelectionMarkerTag = function(A) {
    ...
    var C = B.selection;
    var D;
    try {D = C.createRange();} catch (e) {return null;}
    ;
...
FCKDocumentFragment.prototype = {AppendTo:function(A) {FCKDomTools.MoveChildren(this.RootNode, A);},AppendHtml:function(A) {
...
FCKSelection.GetParentElement = function() {
    switch (this.GetType()) {case 'Control':var A = FCKSelection.GetSelectedElement();return A ? A.parentElement : null;case 'None':return null;default:return this.GetSelection().createRange().parentElement();
    }
};
FCKSelection.GetBoundaryParentElement = function(A) {
    switch (this.GetType()) {case 'Control':var B = FCKSelection.GetSelectedElement();return B ? B.parentElement : null;case 'None':return null;default:var C = FCK.EditorDocument;var D = C.selection.createRange();D.collapse(A !== false);var B = D.parentElement();return FCKTools.GetElementDocument(B) == C ? B : null;
    }
};
...
FCKSelection.HasAncestorNode = function(A) {
    var B;
    if (this.GetSelection().type == "Control") {B = this.GetSelectedElement();} else {
...
FCKSelection.Delete = function() {
    ...
    if (A.type.toLowerCase() != "none") {A.clear();}
    ;
    return A;
};
...
FCKSelection.Save = function() {
    ...
    if (B) {
        C = B.createRange();
        if (C) {if (C.parentElement && FCKTools.GetElementDocument(C.parentElement()) != A) C = null; else if (C.item && FCKTools.GetElementDocument(C.item(0)) != A) C = null;}
    }
    ;
    this.SelectionData = C;
};
FCKSelection._GetSelectionDocument = function(A) {
    var B = A.createRange();
    if (!B) return null; else if (B.item) return FCKTools.GetElementDocument(B.item(0)); else return FCKTools.GetElementDocument(B.parentElement());
};
FCKSelection.Restore = function() {
    if (this.SelectionData) {
        FCK.IsSelectionChangeLocked = true;
        try {
            if (this._GetSelectionDocument(FCK.EditorDocument.selection) == FCK.EditorDocument) return;
...
FCKSelection.Release = function() {delete this.SelectionData;};
var FCKTableHandler = {};
...
FCKTableHandler.GetSelectedCells = function() {
    ...
    if (D && D.tagName.Equals('TD', 'TH')) B[0] = D; else {
        ...
        if (D) {
            for (var i = 0; i < D.cells.length; i++) {
                ...
                if (C.inRange(E) || (C.compareEndPoints('StartToStart', E) >= 0 && C.compareEndPoints('StartToEnd', E) <= 0) || (C.compareEndPoints('EndToStart', E) >= 0 && C.compareEndPoints('EndToEnd', E) <= 0)) {B[B.length] = D.cells[i];}
            }
        }
    }
    ;
...
FCKSpellCheckCommand.prototype.Execute = function() {
    switch (FCKConfig.SpellChecker) {case 'ieSpell':this._RunIeSpell();break;case 'SpellerPages':FCKDialog.OpenDialog('FCKDialog_SpellCheck', 'Spell Check', 'dialog/fck_spellerpages.html', 440, 480);break;
    }
};
FCKSpellCheckCommand.prototype._RunIeSpell = function() {
    ...
    } catch(e) {if (e.number == -2146827859) {if (confirm(FCKLang.IeSpellDownload)) window.open(FCKConfig.IeSpellDownloadUrl, 'IeSpellDownload');} else alert('Error Loading ieSpell: ' + e.message + ' (' + e.number + ')');}
};
